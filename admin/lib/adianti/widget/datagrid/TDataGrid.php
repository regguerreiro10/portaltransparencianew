    ...
    public function createModel( $create_header = true )
    {
        ...
        if ($create_header)
        {
            ...
            if ($this->propertiesEnabled)
            {
                ...
                $visibleColumns = max(0, $totalColumns - $hiddenColumns);
                $badgeClass = ($hiddenColumns === 0) ? 'badge-success' : '';
                $badgeStyle = "position:absolute; top:-8px; right:-8px; min-width:14px; height:14px; border-radius:7px; font-size:9px; line-height:12px; color:#fff; text-align:center;";
                $badgeStyle .= ($hiddenColumns === 0) ? ' width:16px; padding:1px 1px;' : ' padding:1px 4px;';
                $badgeStyle .= ($hiddenColumns === 0) ? '' : ' background-color:#ff8c00;';
                $badgeLabel = ($hiddenColumns === 0) ? '<i class="fa fa-check"></i>' : (string) $visibleColumns;
                
                $button = new TElement('span');
                $button->setProperty('class', $this->propertiesButtonClass . ' tdatagrid-property-btn');
                $button->setProperty('style', trim(($button->{'style'} ?? '') . ';position:relative;'));
                $button->add('<i class="' . $this->propertiesButtonIcon . '"></i>');
                $button->add("<span class='badge {$badgeClass}' style='{$badgeStyle}'>{$badgeLabel}</span>");
