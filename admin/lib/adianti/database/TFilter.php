...
class TFilter extends TExpression
{
    ...
    {
        if ($value instanceof TExpression)
        {
            $origin = 'origem nao identificada';
            $trace = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS);

            foreach ($trace as $frame)
            {
                if (!empty($frame['file']) && stripos($frame['file'], DIRECTORY_SEPARATOR . 'GestaoFrotas' . DIRECTORY_SEPARATOR) !== false)
                {
                    $line = $frame['line'] ?? '?';
            ...
            throw new \InvalidArgumentException("TFilter recebeu {$type} como valor em {$origin}");
        }
