...
trait AdiantiFileSaveTrait
{
    ...
    {
         $input_value = is_object($data) ? ($data->$input_name ?? null) : ($data[$input_name] ?? null);
        ...
        else 
        {
         ...
         //   $dados_file = json_decode(urldecode($input_value));
            $decoded = urldecode(urldecode($input_value));
$dados_file = json_decode($decoded);
