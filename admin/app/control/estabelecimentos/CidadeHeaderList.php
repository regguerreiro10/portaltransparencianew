...
private $form; // form
    ...
    public function __construct($param = null)
    {
        ...
        $basename   = urlencode('cidade-header-list.pdf');
        $download   = "download.php?file=app/manual/cidade-header-list.pdf&basename={$basename}";
        ...
        $panel = new TPanelGroup("Listagem de cidades {$manual}");
        ...
        $dropdown_button_importar->addPostAction( "XLS", new TAction(['CidadeHeaderList', 'onImportarXLS'],['static' => 1]), self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_importar->addPostAction( "MYSQL", new TAction(['CidadeHeaderList', 'onImportarMYSQL'],['static' => 1]), self::$formName, 'fas:database  #614caf' );
        ...
        if(empty($param['target_container']))
        {
          //  $container->add(TBreadCrumb::create(["Fornecedores / Clientes","Cidades"]));
        }
    ...
    { 
        if (isset($param['delete']) && $param['delete'] == 1)
        {
            try
            {
                ...
                $relations = [
                    'PessoaEndereco'     => ['column' => 'cidade_id', 'alias' => 'Pessoa Endereço'],
                    'DepartamentoUnit'   => ['column' => 'cidade_id', 'alias' => 'Departamento Unit']
                ];
                ...
                // Se chegou até aqui, não há relacionamentos bloqueando
