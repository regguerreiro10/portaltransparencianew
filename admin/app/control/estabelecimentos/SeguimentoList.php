...
class SeguimentoList extends TPage
{
...
private $form; // form
    ...
    private static $activeRecord = 'Seguimento';
    private static $primaryKey = 'id';
    private static $formName = 'form_SeguimentoList';
    ...
    public function __construct($param = null)
    {
        ...
        $basename   = urlencode('seguimento-list.pdf');
        $download   = "download.php?file=app/manual/seguimento-list.pdf&basename={$basename}";
        ...
        $this->form->setFormTitle("Listagem de seguimentos {$manual}");
        $this->limit = 20;
        ...
        $btn_onshow = $this->form->addAction("Cadastrar", new TAction(['SeguimentoForm', 'onShow']), 'fas:plus #69aa46');
        ...
        $action_onEdit = new TDataGridAction(array('SeguimentoForm', 'onEdit'));
        ...
        $action_onDelete = new TDataGridAction(array('SeguimentoList', 'onDelete'));
        ...
        $panel = new TPanelGroup("Listagem de seguimentos {$manual}");
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['SeguimentoList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-csv #00b894' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['SeguimentoList', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['SeguimentoList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['SeguimentoList', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
        ...
        $dropdown_button_importar->addPostAction( "XLS", new TAction(['SeguimentoList', 'onImportarXLS'],['static' => 1]), self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_importar->addPostAction( "MYSQL", new TAction(['SeguimentoList', 'onImportarMYSQL'],['static' => 1]), self::$formName, 'fas:database  #614caf' );
        ...
        if(empty($param['target_container']))
        {
       //     $container->add(TBreadCrumb::create(["Fornecedores / Clientes","Seguimentos"]));
        }
    ...
    { 
        if(isset($param['delete']) && $param['delete'] == 1)
        {
            try
            {
                ...
                $relations = [
                    'SeguimentoPessoa' => ['column' => 'seguimento_id', 'alias' => 'Seguimento Pessoa']
                ];
                ...
                // instantiates object
                $object = new Seguimento($key, FALSE); 
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for Seguimento
       ...
       private function executarImportacaoMYSQL()
    {
        try
        {
            ...
            $query = "SELECT id, nome FROM seguimentos ORDER BY id ASC  ";
            $result = $mysqli->query($query);

            while ($row = $result->fetch_assoc())
            {
                // agora sim compara com ID real
                $existe = Seguimento::where('descricao', '=', $row['nome'])
                                ->first();
