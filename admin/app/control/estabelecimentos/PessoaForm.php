...
class PessoaForm extends TPage
{
    ...
    public function __construct( $param )
    {
        ...
        $this->dep = new TCheckList('dep');
        $this->dep->setIdColumn('departamento_unit_id');
        ...
        $pessoa_departamento_pessoa_departamento_unit_id = new TDBCheckGroup('pessoa_departamento_pessoa_departamento_unit_id[]', 'minierp', 'DepartamentoUnit', 'id', '{name}','name asc' , $criteria_pessoa_departamento_pessoa_departamento_unit_id );
        ...
        $categoria_cliente_id->addValidation("Categoria estabelecimento", new TRequiredValidator()); 
       ...
       // $pessoa_departamento_pessoa_departamento_unit_id->enableSearch();
        ...
        $row9 = $tab_668d1f3795fd6->addFields([new TLabel("<br>Ativar selo ambiental ?", null, '14px', null, '100%'),$selo1],[new TLabel("Categoria Estabelecimento:*", '#ff0000', '14px', null, '100%'),$categoria_cliente_id]);
        ...
        $this->dep->addColumn('system_unit_id', 'ID Unit', 'center', '10%');
        $this->dep->addColumn('name_system_unit', 'Unidade', 'left', '45%');
        $this->dep->addColumn('name_departamento_unit', 'Departamento', 'left', '45%');
        $this->dep->addColumn('departamento_unit_id', 'ID', 'center', '10%');

        $this->dep->setHeight(250);
        $this->dep->makeScrollable();
        $this->dep->setSize('100%');

        $this->dep->fillWith('minierp','ViewDepartamentoUnitSystemUnit','departamento_unit_id','name_departamento_unit asc',$criteria_departamento_unit_id);

        $this->form->addField($this->dep);
        ...
        $row15 = $tab_668d1f3795fd6->addContent([$this->dep]);
...
//            $nominatim_url = "https://nominatim.openstreetmap.org/search?q=" . urlencode($endereco) . "&format=json&limit=1";
    ...
    public function onSave($param = null) 
    {
        try
        {
            ...
            $ativoAnterior = null;
            if (!empty($data->id)) {
                $old = new Pessoa($data->id);
                $ativoAnterior = $old->ativo;
            }

            
            // $selo_dir = 'app/documentos/selo';

            if($object->documento && !$data->id)
            {
                if(Pessoa::where('documento', '=', preg_replace('/[^0-9]/', '', $object->documento))->first())
                {
                    throw new Exception('O CNPJ informado já está cadastrado!');
                }
            }
            elseif($object->documento && $data->id)
            {
                if(Pessoa::where('documento', '=', preg_replace('/[^0-9]/', '', $object->documento))->where('id', '!=', $data->id)->first())
                {
                    throw new Exception('O CNPJ informado já está cadastrado!');
                }
            }
            ...
            if($object->email && !$data->id)
            {
                if(Pessoa::where('email', '=', $object->email)->first())
                {
                    throw new Exception('O email informado já está cadastrado!');
                }
            }
            elseif($object->email && $data->id)
            {
                if(Pessoa::where('email', '=', $object->email)->where('id', '!=', $data->id)->first())
                {
                    throw new Exception('O email informado já está cadastrado!');
                }
            }
            ...
            // ✅ Se mudou de Sim -> Não, registra data e notifica SOMENTE a Entidade (SystemUnit)
            if ($object->ativo == 'F' && $ativoAnterior != 'F') {

                if ($object->data_desativacao == null) {
                    $object->data_desativacao = date('Y-m-d H:i:s');
                }

                $codido_email_template_id = EmailTemplate::DESCREDENCIAMENTO;
                ...
                if ($emailTemplate) {
                    ...
                    $unit = new SystemUnit(TSession::getValue('idunit')); // ✅ Entidade marcada

                    // Substituições do template
                    $mensagem = str_replace('{nome}', $object->nome, $mensagem);
                    $mensagem = str_replace('{unidade}', $unit->name, $mensagem);
                    $mensagem = str_replace('{cnpj}', $object->documento, $mensagem);
                    ...
                    $titulo = str_replace('{nome}', $object->nome, $titulo);
                    $titulo = str_replace('{unidade}', $unit->name, $titulo);
                    $titulo = str_replace('{id}', $object->id, $titulo);
                    ...
                    // ✅ Email do gestor responsável (Entidade/SystemUnit)
                    $emailGestor = trim((string) $unit->email);

                    if (!$emailGestor || !filter_var($emailGestor, FILTER_VALIDATE_EMAIL)) {
                        throw new Exception('A Entidade (Unidade) não possui e-mail válido para notificação.');
                    }

                    // MailService::send($emailGestor, $titulo, $mensagem, 'html');
                }

            }
            // ✅ Se estiver ativo, zera a data
            elseif ($object->ativo != 'F') {
            ...
            PessoaDepartamento::where('pessoa_id', '=', $object->id)->delete();

            // grava os selecionados
            $depsSelecionados = $data->dep ?? [];

            if (is_array($depsSelecionados)) {
                foreach ($depsSelecionados as $depId) {
                    $pd = new PessoaDepartamento;
                    $pd->pessoa_id = $object->id;
                    $pd->departamento_unit_id = $depId;
                    $pd->store();
                }
            }
    ...
    public function onEdit( $param )
    {
        try
        {
            if (isset($param['key']))
            {
               ...
               $deps = PessoaDepartamento::where('pessoa_id', '=', $object->id)->getIndexedArray('departamento_unit_id', 'departamento_unit_id');
               $object->dep = array_values((array) $deps);
               $this->form->setData($object);

                //$pessoa_departamento_pessoa_items = $this->storeItems('PessoaDepartamento', 'id', $object, $this->fieldList_668d1b5795fc2, function($masterObject, $detailObject){ 
                ...
                //$this->fieldList_668d1b5795fc2_items = $this->loadItems('ViewDepartamentoUnitSystemUnit', 'pessoa_id', $object, $this->fieldList_668d1b5795fc2, function($masterObject, $detailObject, $objectItems){ 
