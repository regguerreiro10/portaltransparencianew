...
// Definindo a classe
class RelatorioDeNotasFiscais extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        $column_caminho = new TDataGridColumn('caminho', "PDF", 'left');
        $column_peridoServico = new TDataGridColumn('dt_pedido', "Data", 'center');
        $column_pagamentoAutorizado = new TDataGridColumn('nome_estabelecimento', 'Cliente','center');
        ...
        $action_onBaixarNotaFiscal = new TDataGridAction(array($this, 'onBaixarNotaFiscal'));
        $action_onBaixarNotaFiscal->setUseButton(false);
        $action_onBaixarNotaFiscal->setButtonClass('btn btn-default btn-sm');
        $action_onBaixarNotaFiscal->setLabel("Baixar Nota Fiscal");
        $action_onBaixarNotaFiscal->setImage('fa:download rgb(45, 22, 218)');
        $action_onBaixarNotaFiscal->setField('id');

        $this->datagrid->addAction($action_onBaixarNotaFiscal);

        $dropdown_button_exportar = new TDropDown("Exportar", 'fas:file-export #2d3436');
        $dropdown_button_exportar->setPullSide('right');
        $dropdown_button_exportar->setButtonClass('btn btn-default waves-effect dropdown-toggle');
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['RelatorioPorRede', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-csv #00b894' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['RelatorioPorRede', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['RelatorioPorRede', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['RelatorioPorRede', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            $sql = "SELECT DISTINCT  p.id as IdProposta, pf.id, pf.dt_pedido, pf.descricaopedido, 
                    ...
                    ph.estado_pedido_frotas_id, ip.valor_total, ip.valor, ip.tipo, ip.qtde, dp.caminho
                FROM pedido_frotas pf 
                    ...
                    WHERE data_historico is not null AND ph.estado_pedido_frotas_id IN (8,13,18) AND dp.propostas_id is not null AND pf.departamento_unit_id = {$idUnidade};";
            ...
            foreach ($rows as $row)
            {
                ...
                $sqlFinalizado = "SELECT * FROM pedido_frotas_historico WHERE pedido_frotas_id = {$row->id} AND estado_pedido_frotas_id = 13";
                $resultFinalizado = $conn->prepare($sqlFinalizado);
                $resultFinalizado->execute();
                $pedidoFinalizado = $resultFinalizado->fetch(PDO::FETCH_OBJ);
                ...
                if (!isset($groupedData[$idProposta])) {
                    $groupedData[$idProposta] = (object) [
                        ...
                        'aprovador' => $row->name,
                        'dt_aprovacao' => $estaAprovado ? $pedidoFinalizado->data_operacao : "",
    ...
    public function onExportPdf($param = null) 
    {
        try 
        {
            ...
            $reportTitle = isset($param['title']) ? $param['title'] : 'Relatório Por Rede'; // Título dinâmico
    ...
    public function onBaixarNotaFiscal($param)
    {
        $idPedido = (int)$param['id'];
        TTransaction::open('minierp');

        $conn = TTransaction::get();
        $sql = "SELECT DISTINCT dp.caminho
                FROM pedido_frotas pf 
                    LEFT JOIN propostas p ON pf.id = p.pedido_frotas_id 
                    LEFT JOIN itens_propostas ip ON p.id = ip.propostas_id
                    LEFT JOIN propostas_historico ph ON p.id = ph.propostas_id
                    ...
                    LEFT JOIN pessoa pes ON pf.estabelecimento_id = pes.id
                    LEFT JOIN documentos_propostas dp ON p.id = dp.propostas_id
                WHERE data_historico IS NOT NULL AND pf.id = {$idPedido}";
        ...
        if ($rows && isset($rows->caminho)) {
            $caminho = "download.php?file=" . $rows->caminho;
            echo "<script>window.open('$caminho', '_blank');</script>";
            $this->onReload();
        } else {
