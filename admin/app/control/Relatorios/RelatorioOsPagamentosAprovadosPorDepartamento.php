...
// Definindo a classe
class RelatorioOsPagamentosAprovadosPorDepartamento extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        $column_id = new TDataGridColumn('nome', "Empresa | Nome ", 'left');
        $column_fone = new TDataGridColumn('fone', "Contato | E-mail", 'left');
        $column_acao = new TDataGridColumn('caminho', "Ação", 'left');

        $column_id->setTransformer(function ($value, $object, $row) {
            return "<b>Nome:</b> {$object->nome} <br>
                    <b>Documento: </b> {$object->documento}";
        });

        $column_fone->setTransformer(function ($value, $object, $row){
            return "<b> Telefone: </b> {$object->fone} <br>
                    <b> E-mail: </b> {$object->email}";
        });

        $column_acao->setTransformer(function($value, $object, $row) {
            $botoes = '';
            
            $botoes .= "<a href='index.php?class=RelatorioOsPagamentosAprovados&method=onReload&key={$object->id}&id={$object->id}' target='_blank' class='btn btn-sm btn-success' style='margin:2px'>Visualizar</a>";
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['RelatorioOsPagamentosAprovadosPorDepartamento', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-csv #00b894' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['RelatorioOsPagamentosAprovadosPorDepartamento', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['RelatorioOsPagamentosAprovadosPorDepartamento', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['RelatorioOsPagamentosAprovadosPorDepartamento', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            $sql = "SELECT * from pessoa WHERE tipo_cliente_id = 2;";
    ...
    public function onSearch($param)
    {
        ...
        try
        {
            ...
            $sql = "SELECT DISTINCT  p.id as IdProposta, pf.id, pf.dt_pedido, pf.descricaopedido, 
                    ...
                    LEFT JOIN pessoa pes ON pf.estabelecimento_id = pes.id
                    LEFT JOIN veiculos v ON pf.veiculos_id = v.id
                    WHERE data_historico is not null AND ph.estado_pedido_frotas_id IN (8,13,18)";
