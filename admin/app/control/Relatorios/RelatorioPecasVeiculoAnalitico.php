...
// Definindo a classe
class RelatorioPecasVeiculoAnalitico extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        $dateDe->addItems($itens);

        $row1 = $this->form->addFields([new TLabel("Placa:", null, '14px', null, '100%'),$placa],[new TLabel("Date de:", null, '14px', null, '100%'),$dateDe]);
        ...
        $column_id = new TDataGridColumn('id', "OS", 'center');
        $column_status = new TDataGridColumn('status', "Status", 'center');
        $column_data_abertura = new TDataGridColumn('dt_pedido', "Data Abertura", 'left');
        $column_data_finalizacao = new TDataGridColumn('dt_finalizacao', "Data Finalização", 'center');
        $column_data_pg_autorizado = new TDataGridColumn('data_pg_autorizado', "Data Pagamento Autorizado", 'center');
        ...
        $column_qtdeServico = new TDataGridColumn('qtd_servico', 'Qnt.Serviço', 'center');
        $column_valorUniServico = new TDataGridColumn('valor_uni_servico', 'Valor Uni. Serviço', 'center');
        $column_desconto = new TDataGridColumn('desconto', 'Desconto', 'center');
        $column_subtotal = new TDataGridColumn('valor_total', 'SubTotal', 'center');
        $column_totalComDesconto = new TDataGridColumn('valor_total_desconto', 'Total Com Desconto', 'center');
        $column_garantia = new TDataGridColumn('garantia', 'Garantia', 'center');
        ...
        $column_id->setTransformer(function ($value, $object, $row){
            ...
            return "<b>Pedido:</b> {$object->id} <br>
                    <b>Proposta:</b> {$object->IdProposta}";
        });
        ...
        $column_totalComDesconto->setTransformer(function ($value) {
            return is_numeric($value) ? 'R$ ' . number_format((float) $value, 2, ',', '.') : '';
        });
        ...
        $column_data_finalizacao->setTransformer(function ($value, $object, $row){

            if (!isset($object->dt_finalizacao) || empty($object->dt_finalizacao)) {
                return "00/00/0000";
            }
        
            return date("d/m/Y", strtotime($object->dt_finalizacao));
        });
        
        $column_data_pg_autorizado->setTransformer(function ($value, $object, $row){
 
            if (!isset($object->data_pg_autorizado) || empty($object->data_pg_autorizado)) {
                return "00/00/0000";
            }
        
            return date("d/m/Y", strtotime($object->data_pg_autorizado));
        });
        ...
        $this->datagrid->addColumn($column_data_finalizacao);
        $this->datagrid->addColumn($column_data_pg_autorizado);
        ...
        $this->datagrid->addColumn($column_qtdeServico);
        ...
        $this->datagrid->addColumn($column_totalComDesconto);
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['RelatorioPecasVeiculoAnalitico', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-csv #00b894' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['RelatorioPecasVeiculoAnalitico', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['RelatorioPecasVeiculoAnalitico', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['RelatorioPecasVeiculoAnalitico', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            $sql = "SELECT DISTINCT  p.id as IdProposta, p.desconto_contratual, pf.id, pf.dt_pedido, pf.descricaopedido, 
                    ...
                    ph.estado_pedido_frotas_id, ip.valor_total, ip.valor, prod.nome AS pecaItem, ip.tipo, ip.qtde,
            ...
            foreach ($rows as $row)
            {
                ...
                if (!isset($groupedData[$idProposta])) {
                    $groupedData[$idProposta] = (object) [
                        ...
                        'qtd_servico' => '',
                        'valor_uni_peca' => '',
                        'desconto' => $row->desconto_contratual == null ? 0 : $row->desconto_contratual,
    ...
    public function onSearch($param)
    {
        ...
        try
        {
            ...
            $count_sql = "SELECT DISTINCT  count(p.id)
                    FROM pedido_frotas pf 
                    LEFT JOIN propostas p ON pf.id = p.pedido_frotas_id 
                    LEFT JOIN itens_propostas ip ON p.id = ip.propostas_id
                    LEFT JOIN produto prod on prod.id=ip.produto_id                   
                    ...
                    LEFT JOIN modelo m ON v.modelo_id = m.id
                    LEFT JOIN estado_pedido_frotas epf ON pf.estado_pedido_frotas_id = epf.id 
                    where pf.system_unit_id = {$idUnidade}";
            ...
            $sql = "SELECT DISTINCT  p.id as IdProposta, p.desconto_contratual, pf.id, pf.dt_pedido, pf.descricaopedido, 
                    pf.estabelecimento_id, epf.nome AS statusPedido, pes.nome, pf.veiculos_id, v.placa, m.descricao as descricaoModelo, v.hodometroatual, 
                    pf.dt_finalizacao, (select ph.data_historico from propostas_historico ph where ph.propostas_id=p.id and ph.estado_pedido_frotas_id=18 limit 0,1) as data_historico, (select ph.aprovador_frotas_id from propostas_historico ph where ph.propostas_id=p.id and ph.estado_pedido_frotas_id=18 limit 0,1) as aprovador_frotas_id,
                    (select su.name from propostas_historico ph 
                     LEFT JOIN system_users su ON ph.aprovador_frotas_id = su.id
                     where ph.propostas_id=p.id and ph.estado_pedido_frotas_id=18 limit 0,1) as name, 
                   pf.estado_pedido_frotas_id, ip.valor_total, ip.valor, prod.nome AS pecaItem, ip.tipo, ip.qtde,
                    ip.diasdegarantia, ip.perc_desconto
                    FROM pedido_frotas pf 
                    LEFT JOIN propostas p ON pf.id = p.pedido_frotas_id 
                    LEFT JOIN itens_propostas ip ON p.id = ip.propostas_id
                    ...
                    LEFT JOIN modelo m ON v.modelo_id = m.id
                    LEFT JOIN estado_pedido_frotas epf ON pf.estado_pedido_frotas_id = epf.id
                   AND pf.system_unit_id = {$idUnidade}";
            ...
            else if(!empty($data->dataDe) && $data->dataDe == '1')
            {
                $sql .= " AND pf.dt_finalizacao BETWEEN '{$data->dt_pedido}' AND '{$data->dt_pedido_final}'";
            }
            else if(!empty($data->dataDe) && $data->dataDe == '2')
            {
                $sql .= " AND ph.data_historico BETWEEN '{$data->dt_pedido}' AND '{$data->dt_pedido_final}'";
            }
    ...
    public function onExportPdf($param = null) 
    {
        try 
        {
            ...
            $reportTitle = isset($param['title']) ? $param['title'] : 'Relatório Peças Veículo Analítico'; // Título dinâmico
