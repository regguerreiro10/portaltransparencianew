...
class PedidoRelFrotasList extends TPage
{
    private $form; // form
    ...
    private static $formName = 'form_PedidoRelFrotasList';
    private $showMethods = ['onReload', 'onSearch', 'onRefresh', 'onClearFilters', 'onGlobalSearch'];
    private $limit = 10;

    use BuilderDatagridTrait;

    private function getEstadosPermitidosRelatorio()
    {
        return [
    ...
    private function getAllowedUnitIdsByEntidade($entidadeId)
    {
        ...
        $links = EntidadeSystemUnit::where('entidade_id', '=', $entidadeId)
        ...
        if ($links)
        {
            ...
            {
                if (!empty($link->system_unit_id))
                {
                    $unitIds[] = (int) $link->system_unit_id;
                }
            }
        }
    ...
    public function __construct($param = null)
    {
        ...
        $this->form->setFormTitle("Relatorio Consolidade de Pedidos de Frotas");
        $this->limit = 10;
        ...
        $criteria_estado_pedido_frotas_id->add(new TFilter('id', 'in', $this->getEstadosPermitidosRelatorio()));
        $criteria_veiculos_id = new TCriteria();
        $allowedUnitIds = $this->getAllowedUnitIdsByEntidade(TSession::getValue('entidade'));
        $criteria_veiculos_id->add(new TFilter('status_veiculo_id', '=', 1));
        if (!empty($allowedUnitIds))
        {
            $criteria_veiculos_id->add(new TFilter('system_unit_id', 'in', $allowedUnitIds));
        }
        else
        {
            $criteria_veiculos_id->add(new TFilter('id', '=', -1));
        }
        ...
        $criteria_departamento_unit_id->add(
            new TFilter(
                'id',
                'in',
                "(SELECT system_unit_id FROM entidade_system_unit WHERE entidade_id = {$entidadeid})"
            )
        );
        $filterVar = GrupoPessoa::FORNECEDOR;
        $criteria_estabelecimento_id->add(new TFilter('id', 'in', "(SELECT id FROM pessoa WHERE  deleted_at is null AND id = '{$filterVar}')")); 
        $login = new LoginForm([]);
        // $AlertMensagem = $login->onMensagem('PedidoRelFrotasList');
        ...
        $system_unit_id = new TDBSelect('system_unit_id', 'minierp', 'SystemUnit', 'id', 'name', 'name asc', $criteria_departamento_unit_id);
        ...
        $row3 = $this->form->addFields([new TLabel("Tipo de Manutenção:", null, '14px', null, '100%'),$tipo_manutencao_id],[new TLabel("Unidade", null, '14px', null, '100%'),$system_unit_id]);
...
//             $action9 = new TDataGridAction(['PedidoRelFrotasList', 'onGerarFinanceiro'],   ['id' => '{id}']);
//         } else {
//             $action9 = new TDataGridAction(['PedidoRelFrotasList', 'onGerarFinanceiroItem'],   ['id' => '{id}']);
//         }
...
//    //     $action13 = new TDataGridAction(['PedidoRelFrotasList', 'onImprimir'],   ['id' => '{id}']);
...
//        $action1->setDisplayCondition('PedidoRelFrotasList::onExibirView');

//         $action2->setLabel('Editar');
//         $action2->setImage('far:edit #478fca');
//        $action2->setDisplayCondition('PedidoRelFrotasList::onExibirEditar');

//         $action3->setLabel('Excluir');
//         $action3->setImage('fas:trash-alt #dd5a43');
//         $action3->setDisplayCondition('PedidoRelFrotasList::onExibirExcluir');
...
//    //    $action13->setDisplayCondition('PedidoRelFrotasList::onExibirDocCotacao');

//          $action16->setLabel('Detalhes Propostas');
//         $action16->setImage('fas:plus #69aa46');
// //       $action16->setDisplayCondition('PedidoRelFrotasList::onExibirDocCotacao');

//         $action5->setLabel('Gerar Proposta');
//         $action5->setImage('fas:envelope #E91E63');
//        $action5->setDisplayCondition('PedidoRelFrotasList::onExibirEnvio');

//      //   $action61->setLabel('Pré-Aprovar');
//      //   $action61->setImage('far:thumbs-up #9C27B0');
//      //  $action61->setDisplayCondition('PedidoRelFrotasList::onExibirPreAprovar');

//     //   $action6->setLabel('Aprovar');
//     //    $action6->setImage('fas:thumbs-up #9C27B0');
//     //   $action6->setDisplayCondition('PedidoRelFrotasList::onExibirAprovar');

//    //     $action7->setLabel('Reprovar');
//    //     $action7->setImage('fas:thumbs-down #F44336');
//   //     $action7->setDisplayCondition('PedidoRelFrotasList::onExibirReprovar');

//         $action8->setLabel('Cancelar pedido');
//         $action8->setImage('fas:times-circle #E91E63');
//        $action8->setDisplayCondition('PedidoRelFrotasList::onExibirCancelado');

//         $action9->setLabel('Gerar financeiro');
//         $action9->setImage('fas:money-bill-wave #FFA500');
//        $action9->setDisplayCondition('PedidoRelFrotasList::onExibirGerarFinanceiro');

//         $action10->setLabel('Finalizar pedido');
//         $action10->setImage('fas:door-closed #009688');
//         $action10->setDisplayCondition('PedidoRelFrotasList::onExibirFinalizar');

//         $action11->setLabel('Anexos');
//         $action11->setImage('fas:paperclip #795548');
//        $action11->setDisplayCondition('PedidoRelFrotasList::onExibirAnexos');



//         $action12->setLabel('Cancelar Aprovação');
//         $action12->setImage('fas:undo #009688');
//        $action12->setDisplayCondition('PedidoRelFrotasList::onExibirCancelarAprovacao');

//        $action14->setLabel('Autorizar Pedido');
//        $action14->setImage('fas:unlock #ffc83d');
//        $action14->setDisplayCondition('PedidoRelFrotasList::onExibirAutorizacao');

//  //       $action15->setLabel('Saída Veículo');
//  //       $action15->setImage('fas:arrow-left #F44336');
//  //      $action15->setDisplayCondition('PedidoRelFrotasList::onExibirRetiradaVeiculo');
...
//         $this->datagrid->addActionGroup($action_group);
        ...
        $panel = new TPanelGroup("Relatorio Consolidade de Pedidos de Frotas");
     ...
     //   $this->datagrid_form->class = ' table-fixed-header ';
       // $this->datagrid_form->style = ' height:550px;';
        ...
        $btnShowCurtainFilters->setAction(new TAction(['PedidoRelFrotasList', 'onShowCurtainFilters']), "Filtros");
        ...
        $button_limpar_filtros->setAction(new TAction(['PedidoRelFrotasList', 'onClearFilters']), "Limpar filtros");
        ...
        $button_atualizar->setAction(new TAction(['PedidoRelFrotasList', 'onRefresh']), "Atualizar");
...
//         $this->datagrid_form->addField($button_cilia);
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['PedidoRelFrotasList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-csv #00b894' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['PedidoRelFrotasList', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['PedidoRelFrotasList', 'onExportPdfRel'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['PedidoRelFrotasList', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
        $dropdown_button_exportar->addPostAction( "PDF/HTML", new TAction(['PedidoRelFrotasList', 'onExportHtml'],['static' => 1]), 'datagrid_'.self::$formName, 'fab:html5 #E34F26'  );
        ...
        // $dropdown_button_importar->addPostAction( "XLS", new TAction(['PedidoRelFrotasList', 'onImportarXLS'],['static' => 1]), self::$formName, 'fas:file-excel #4CAF50' );
        // $dropdown_button_importar->addPostAction( "MYSQL", new TAction(['PedidoRelFrotasList', 'onImportarMYSQL'],['static' => 1]), self::$formName, 'fas:database  #614caf' );
    ...
    public function onExportPdfRel($param = null) 
    {
        try
        {
            ...
            // Regras fixas do onReload
            $criteria->add(new TFilter('estado_pedido_frotas_id', 'in', $this->getEstadosPermitidosRelatorio()));

            $userid = TSession::getValue('userid');
            $entidadeId = TSession::getValue('entidade');
            $allowedUnitIds = $this->getAllowedUnitIdsByEntidade($entidadeId);
            ...
            if (!empty($allowedUnitIds))
            {
                $criteria->add(new TFilter('system_unit_id', 'in', $allowedUnitIds));
            }
            else
            {
            ...
            $units = new Entidade(TSession::getValue('entidade'));
            if ($units) {
                $cnpj = $units->cnpj;
                $unidade = $units->nome;
             } else {
            ...
            //email
               ...
               $dataPedido = TDate::date2br($object->dt_pedido);

               $pdf->SetXY(47,$alturalinha);
               $pdf->Cell(70,5,$dataPedido,0,1,'L');

               $pessoa = new Pessoa($object->estabelecimento_id);
               $pdf->SetXY(85,$alturalinha);
               //$pdf->Cell(70,5,$pessoa->nome,0,1,'L');
               $pdf->Cell(70,5,mb_convert_encoding(substr($pessoa->nome,0,36), 'ISO-8859-1', 'UTF-8'),0,1,'L');

               $pdf->SetXY(225,$alturalinha);
               $pdf->Cell(70,5,mb_convert_encoding(substr($object->placa .' - '.$object->descricaopedido,0,50), 'ISO-8859-1', 'UTF-8'),0,1,'L');

               $estadopedido = new EstadoPedidoFrotas($object->estado_pedido_frotas_id);
               $pdf->SetXY(432,$alturalinha);
               ...
               $pdf->Cell(70,5,number_format($object->valor_desconto_proposta, 2),0,1,'R');

               $pdf->SetXY(545,$alturalinha);
               $pdf->Cell(70,5,number_format($object->valor_liquido_proposta, 2),0,1,'R');
               $pdf->setFont('arial','',5);
   ...
   private function cabecalho($pdf, $linha,$pag, $unidade, $cnpj, $filters)
    {
        ...
        {
            $pdf->Image($logoPath, 26, 7, 46);
        }
        $pdf->SetFont('arial','B',8);
        $pdf->SetXY(80,14);
        $pdf->Cell(540,5, mb_convert_encoding(substr((string) $unidade, 0, 70), 'ISO-8859-1', 'UTF-8'),0,1,'L');
        $pdf->SetXY(131,28);
        $pdf->Cell(70,5,mb_convert_encoding('Relatorio Consolidade de Pedidos de Frotas', 'ISO-8859-1', 'UTF-8'),0,1,'C');
        $pdf->SetXY(660,14);
        ...
        $pdf->SetXY(80,42);
        $pdf->Cell(560,5,$cnpj.'      '. $label,0,1,'L');
        $pdf->SetXY(748,42);
        ...
        $pdf->SetXY(225,62);
        $pdf->Cell(100,5,utf8_decode('Placa / Descrição do pedido'),0,1,'L');
    ...
    public function onSearch($param = null)
    {
        ...
        {
            if (is_array($data->system_unit_id))
            {
                $unitIds = array_values(array_filter(array_map('intval', $data->system_unit_id), fn($id) => $id > 0));
                if (!empty($unitIds))
                {
                    $filters[] = new TFilter('system_unit_id', 'in', $unitIds);
                }
            }
            else
            {
                $filters[] = new TFilter('system_unit_id', '=', (int) $data->system_unit_id);
            }
        }
        ...
        {
            $estadosPermitidos = $this->getEstadosPermitidosRelatorio();
            $estadoSelecionado = is_array($data->estado_pedido_frotas_id)
                ? array_values(array_intersect(array_map('intval', $data->estado_pedido_frotas_id), $estadosPermitidos))
                : [(int) $data->estado_pedido_frotas_id];
            
            if (!empty($estadoSelecionado))
            {
                $filters[] = new TFilter('estado_pedido_frotas_id', 'in', $estadoSelecionado);
            }
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            {
                $param['order'] = 'entidade_id, system_unit_id, departamento_unit_id, id';
            }
            ...
            // Regra fixa do relatório: somente estados aprovados/entregues/pagto aprovado/finalizado
            $criteria->add(new TFilter('estado_pedido_frotas_id', 'in', $this->getEstadosPermitidosRelatorio()));
         ...
         //   $criteria = new TCriteria();
            ...
            // Sempre aplica o filtro das unidades vinculadas à entidade logada
            if (!empty($allowedUnitIds))
            {
                $criteria->add(new TFilter('system_unit_id', 'in', $allowedUnitIds));
            }
            ...
            try {
               ...
               // $conexao   = TTransaction::get(); 
                ...
                if ($fornecedores) { 
                    ...
                    TApplication::loadPage('PedidoRelFrotasList', 'onSetProject');
                } else {
    ...
    public static function onShowCurtainFilters($param = null) 
    {
        try 
        {
            ...
            $page->setProperty('page-name', 'PedidoRelFrotasListSearch');
            $page->setProperty('page_name', 'PedidoRelFrotasListSearch');
 ...
 public function onExportHtml($param = null)
{
    try
    {
        ...
        if ((!file_exists($output) && is_writable(dirname($output))) || is_writable($output))
        {
            ...
            if (file_exists($logoPath))
            {
                $logoHtml = "<img src='../images/logo.png' alt='Logo' style='height:52px;'>";
            }
            ...
            if ($entidadeId > 0)
            {
                ...
                if (!empty($unitNames))
                {
                    ...
                    foreach ($unitNames as $name)
                    {
                        ...
                        $items .= "<li>{$safeName}</li>";
                    }

                    $legendHtml = "
<div style='margin-top:18px; border-top:1px solid #ddd; padding-top:10px; font-size:12px;'>
<strong>Legenda - Entidade/System Unit</strong>
...
{$logoHtml}
<h2 style='margin:0; font-size:20px;'>Relatorio Consolidade de Pedidos de Frotas</h2>
</div>
{$html->getContents()}
{$legendHtml}
