...
class FinanceiroCategoriaList extends TPage
{
    private $datagrid;
    ...
    private static $formName = 'formList_FinanceiroCategoria';
    private $limit = 20;
    ...
    public function __construct($param = null)
    {
        ...
        $actionEdit = new TDataGridAction(['FinanceiroCategoriaForm', 'onEdit']);
        ...
        $panel = new TPanelGroup('Categorias financeiras');
        ...
        $buttonCadastrar = new TButton('button_cadastrar_financeiro_categoria');
        $buttonCadastrar->setAction(new TAction(['FinanceiroCategoriaForm', 'onShow']), 'Cadastrar');
        ...
        if (empty($param['target_container'])) {
            $container->add(TBreadCrumb::create(['Modulo Financeiro', 'Categorias']));
        }
        $container->add($panel);
    ...
    public function onReload($param = null)
    {
        try {
            ...
            $criteria->setProperty('order', $param['order'] ?? 'nome');
    ...
    public function onDelete($param = null)
    {
        if (isset($param['delete']) && $param['delete'] == 1) {
            try {
                TTransaction::open(self::$database);
                FinanceiroPublicoSchemaHelper::ensureSchema();
                $object = new FinanceiroCategoria($param['key'], false);
                $object->delete();
                TTransaction::close();
                $this->onReload($param);
