...
class PedidoSaldoDepartamentoList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $column_valor_liquido_cotacao_transformed = new TDataGridColumn('valor_liquido_cotacao', "Valor liquido cotação", 'left');
        $column_estado_pedido_venda_nome = new TDataGridColumn('estado_pedido_venda->nome', "Estado pedido", 'left');
                ...
                if (TSession::getValue('aprovacao_por_item') == 1) {
                    ...
                    $cot = Cotacao::where('pedido_id', '=', $object->id)
                        ->where('estado_pedido_id', 'IN', [
                            ...
                            EstadoPedido::COMPROPOSTA,
                            EstadoPedido::ENTREGUE,
                            EstadoPedido::REVISAO,
        ...
        $column_estado_pedido_venda_nome->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            ...
            if ($temnotafiscal) {
                ...
                $anexo = $object->estado_pedido_venda->nome;
            }

            return "<span class='label label-default' style='width:260px; background-color:{$object->estado_pedido_venda->cor}; display:inline-block;'> {$anexo} {$revisao} </span>";
        ...
        {
            if ($object->valor_liquido_cotacao === null || $object->valor_liquido_cotacao === '')
            {
                $value = $object->valor_total;
            }
            if(!$value)
            {
                $value = 0;
        ...
        // $action_onDelete = new TDataGridAction(array('PedidoSaldoDepartamentoList', 'onImprimirPedido'));
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if($filters = TSession::getValue(__CLASS__.'_filters'))
            {
                ...
                $criteria->add(new TFilter('saldo_departamento_id','=',TSession::getValue('idsaldodepartamento')));
