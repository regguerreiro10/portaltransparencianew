...
class ViewRedesdisponiveisComprasList extends TPage
{
    private $form; // form
    ...
    private static $formName = 'form_ViewRedesdisponiveisComprasList';
    ...
    public function __construct($param = null)
    {
        ...
        $this->embeddedMode = !empty(TSession::getValue('pedido_frotas_id')) || !empty($param['target_container']);
        ...
        $seguimento_id = new TDBCombo('seguimento_id', 'minierp', 'Seguimento', 'id', 'descricao','descricao asc' , $criteria_seguimento_id );
        ...
        //</onBeforeAddFieldsToForm>
        $row1 = $this->form->addFields([new TLabel("Segmento:", null, '14px', null, '100%'),$seguimento_id],[new TLabel("Cidade:", null, '14px', null, '100%'),$cidade_id]);
        ...
        $column_pessoa_seguimento->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            ...
            {
                return $this->seguimentoMap[$pessoaId];
            }
            return 'Não informado!';
        });
    ...
    public function onSearch($param = null)
    {
        ...
        // Filtro por seguimento_id (SELECT IN na tabela seguimento_pessoa)
        ...
        {
            TSession::setValue('filtroseguimento_id', $data->seguimento_id);

            try {
                TTransaction::open('minierp'); // Substitua pelo nome correto da conexão

                $criteria = new TCriteria;
                $criteria->add(new TFilter('seguimento_id', '=', $data->seguimento_id));

                $repository = new TRepository('SeguimentoPessoa'); // Substitua pelo nome correto da classe AR
                ...
                if ($registros)
                {
                    ...
                    {
                        $ids[] = $item->pessoa_id; // ou 'id_pessoa' conforme o nome do campo
                    }
                }
                ...
                if (!empty($ids))
                {
                    $filters[] = new TFilter('id', 'IN', $ids); // Supondo que o filtro é na tabela de pessoas
                }
                else
                {
                    $filters[] = new TFilter('id', '=', 0); // Garante que nenhum resultado será retornado
                }

            } catch (Exception $e) {
                new TMessage('error', 'Erro ao buscar seguimentos: ' . $e->getMessage());
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if (TSession::getValue('pedido_compra_id')) {
                try {
                    $pedido_compra_id = TSession::getValue('pedido_compra_id');

                    $criteriaChecked = new TCriteria();
                    $criteriaChecked->add(new TFilter('pedido_id', '=', $pedido_compra_id));

                    $repoChecked = new TRepository('PedidocompraAsCliente');
            ...
            if (empty(TSession::getValue('pedido_compra_id')))
            {
    ...
    private function preloadSeguimentoMap($objects)
    {
        ...
        $seguimentoIds = [];
        $primeiroSeguimentoPorPessoa = [];
        foreach ($seguimentosPessoa as $item)
        {
            $pessoaId = (int) ($item->pessoa_id ?? 0);
            $seguimentoId = (int) ($item->seguimento_id ?? 0);

            if (!$pessoaId || !$seguimentoId || isset($primeiroSeguimentoPorPessoa[$pessoaId]))
            {
                continue;
            }

            $primeiroSeguimentoPorPessoa[$pessoaId] = $seguimentoId;
        ...
        foreach ($primeiroSeguimentoPorPessoa as $pessoaId => $seguimentoId)
        {
            if (!empty($seguimentoDescricaoMap[$seguimentoId]))
            {
                $this->seguimentoMap[$pessoaId] = $seguimentoDescricaoMap[$seguimentoId];
            }
        }
    }
