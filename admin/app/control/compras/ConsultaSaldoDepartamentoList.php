...
class ConsultaSaldoDepartamentoList extends TPage
{
...
private $form; // form
    ...
    private static $formName = 'form_ConsultaSaldoDepartamentoList';
    ...
    public function __construct($param = null)
    {
        ...
        $basename   = urlencode('dotacao-orcamentaria-list.pdf');
        $download   = "download.php?file=app/manual/dotacao-orcamentaria-list.pdf&basename={$basename}";
        ...
        $this->form->setFormTitle("Consulta Saldos de Dotações e Valores Empenhados {$manual}");
        $this->limit = 0;
        ...
        $column_total_pedido->setTransformer(function($value, $object)
        {
            ...
            $pedido = Pedido::where('saldo_departamento_id', '=', $object->id)
                            ->where('estado_pedido_venda_id', 'not in', [EstadoPedidoVenda::CANCELADO, EstadoPedidoVenda::REPROVADO])
                            ->load();
            if ($pedido) {
               foreach ($pedido as $ped) {
                    if ($ped->valor_liquido_cotacao === null || $ped->valor_liquido_cotacao === '')
                    {
                        $value =  $value +  $ped->valor_total;
                    }
                    else
                    {
                        $value = $value + $ped->valor_liquido_cotacao;
                    }
                }
            }
       ...
       $column_qtde_pedidos->setTransformer(function($value, $object)
        {
            ...
            $pedidos = Pedido::where('saldo_departamento_id', '=', $object->id)->load();
          ...
          $column_saldo_atual->setTransformer(function($value, $object)
        {
            ...
            $pedido = Pedido::where('saldo_departamento_id', '=', $object->id)
                            // ->where('estado_pedido_venda_id', 'in', [8,1,13,18,20])
                            ->load();
            if ($pedido) {
               foreach ($pedido as $ped) {
                    if (($ped->estado_pedido_venda_id == EstadoPedido::APROVADO) || 
                       ($ped->estado_pedido_venda_id == EstadoPedido::ENTREGUE) || 
                       ($ped->estado_pedido_venda_id == EstadoPedido::FINALIZADO) || 
                       ($ped->estado_pedido_venda_id == EstadoPedido::PGTOAPROVADO) || 
                       ($ped->estado_pedido_venda_id == EstadoPedido::PREAPROVADO)) {
                    if ($ped->valor_liquido_cotacao === null || $ped->valor_liquido_cotacao === '')
                    {
                        $value =  $value +  $ped->valor_total;
                    }
        ...
        $action_onExibirDetalhesPedido = new TDataGridAction(array('ConsultaSaldoDepartamentoList', 'onExibirDetalhesPedido'));
        ...
        $panel = new TPanelGroup("Consulta Saldos de Dotações e Valores Empenhados {$manual}");
        ...
        $btnShowCurtainFilters->setAction(new TAction(['ConsultaSaldoDepartamentoList', 'onShowCurtainFilters']), "Filtros");
        ...
        $button_limpar_filtros->setAction(new TAction(['ConsultaSaldoDepartamentoList', 'onClearFilters']), "Limpar filtros");
        ...
        $button_atualizar->setAction(new TAction(['ConsultaSaldoDepartamentoList', 'onRefresh']), "Atualizar");
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['ConsultaSaldoDepartamentoList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-csv #00b894' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['ConsultaSaldoDepartamentoList', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['ConsultaSaldoDepartamentoList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['ConsultaSaldoDepartamentoList', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
    ...
    public function onExibirDetalhesPedido($param = null) 
    {
        try 
        {
            //code here
            $pageParam = [];
            TSession::setValue('idsaldodepartamento', null);
            TSession::setValue('idsaldodepartamento', $param['key']);
            TApplication::loadPage('PedidoSaldoDepartamentoList', 'onShow', [
    ...
    public static function onShowCurtainFilters($param = null) 
    {
        try 
        {
            ...
            $page->setProperty('page-name', 'ConsultaSaldoDepartamentoListSearch');
            $page->setProperty('page_name', 'ConsultaSaldoDepartamentoListSearch');
