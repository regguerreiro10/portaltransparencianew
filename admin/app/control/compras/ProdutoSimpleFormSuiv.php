...
class ProdutoSimpleFormSuiv extends TPage
{
    ...
    public static function onQuickSave($param = null)
    {
        ...
        {        
            ...
            $object->familia_produto_id = $familia_produto_id;
            $object->ativo              = 'T';
            $object->nome               = BComboNoResultsService::getQuickFieldValue($param);

            $object->store();

            // Atualiza o componente do BCombo (onde você digitou o novo produto)
            BComboNoResultsService::handleRefreshComponent($param, $object);

             //    se quiser garantir que o familia_produto_id do form está certo
            $data = new stdClass;
            $data->familia_produto_id = TSession::getValue('familia_produto_id');
            $data->produto_id         = $object->id;
            TForm::sendData('ItensPedidoFrotasProdutoForm', $data, false, false);

            if (TSession::getValue('tipo') == 1) {
                ItensPedidoFrotasProdutoForm::fireEvents($data);
                ItensPropostasProdutosForm::fireEvents($data);
            } else {
               ItensPedidoFrotasServicoForm::fireEvents($data);
               ItensPropostasServicosForm::fireEvents($data);
            }
