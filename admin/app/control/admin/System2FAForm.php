...
class System2FAForm extends TPage
{
    public function __construct($param = null)
    {
        try
        {
            ...
            if (isset($param['email'])) {
                $email_login = trim((string) $param['email']);
            } else {
                $email_login = trim((string) TSession::getValue('email_login'));
            }            
            $login       = trim((string) TSession::getValue('login'));

            if (!empty($email_login))
            {
                $user = SystemUsers::where('email', '=', $email_login)->first();
            }
            else
            {
    ...
    public function onEditarUsuario($param)
    {
          TSession::setValue('email_login', null);
            TSession::setValue('email_login',$param['email'] ?? null);
            $this->onShow($param);
    }
