...
class PropostasDisponiveisList extends TPage
{
...
private $form; // form
    ...
    public function __construct($param = null)
    {
        ...
        $download   = "download.php?file=app/manual/propostas-disponiveis-list.pdf&basename={$basename}";
        ...
        $this->form->setFormTitle("Listagem de propostas {$manual}");
        $this->limit = 20;
...
$AlertMensagem = $login->onMensagem('PropostasDisponiveisList');
        ...
        $row3 = $this->form->addFields([new TLabel("Estado pedido frotas:", null, '14px', null, '100%'),$estado_pedido_frotas_id],[new TLabel("Veiculos, Aeronaves e/ou Equipamentos :", null, '14px', null, '100%'),$veiculos_id]);
      ...
      // keep the form filled during navigation with session data
        ...
        $column_veiculos_marca_id = new TDataGridColumn('veiculos_id', "Marca", 'left');
        $column_veiculos_modelo_id = new TDataGridColumn('veiculos_id', "Modelo", 'left');
     ...
     //   $column_valor_desconto_transformed = new TDataGridColumn('valor_desconto', "Valor desconto", 'left');
      //  $column_valor_liquido_transformed = new TDataGridColumn('valor_liquido', "Valor liquido", 'left');
        ...
        $column_numero_orcamento_base = new TDataGridColumn('numero_orcamento_base', "Nº Orçamento base", 'center', '160px');
        ...
        $column_veiculos_id->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
          ...
          $column_veiculos_marca_id->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            TTransaction::open('minierp');
          ...
          $column_veiculos_modelo_id->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            TTransaction::open('minierp');
       ...
       $column_valor_total_transformed->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            ...
            if (isset($object->total_geral_com_desconto) && is_numeric($object->total_geral_com_desconto)) {
                $total = (float) $object->total_geral_com_desconto;
            } elseif (isset($object->valor_liquido) && is_numeric($object->valor_liquido)) {
                $total = (float) $object->valor_liquido;
      ...
      /*  $column_comentario_proposta_propostas_to_string_transformed->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            ...
            // Cria a aÃ§Ã£o que aponta para o PropostasForm::onEdit com o ID do objeto
        ...
        $column_numero_orcamento_base->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            if (empty($object->orcamento_base) || (int)$object->orcamento_base !== 1) {
                return '-';
            }

            if (empty($value)) {
                return "<span class='label label-default' style='background-color:#ffc107;color:#000; width:120px; display:inline-block; text-align:center;'>NÃƒO INFORMADO</span>";
            }
            return (string) $value;
        });
        ...
        $this->datagrid->addColumn($column_veiculos_marca_id);
        $this->datagrid->addColumn($column_veiculos_modelo_id);
...
/*
        ...
        $action_onEdit->setDisplayCondition('PropostasDisponiveisList::onExibirEditar');
     ...
     */
     // creates two datagrid actions
        ...
        $action8 = new TDataGridAction(['PropostasDisponiveisList', 'onReprovar'],     ['id' => '{id}']);
        ...
        $action8->setLabel('Rejeitar');
        $action8->setImage('fas:thumbs-down #F44336');
        $action8->setDisplayCondition('PropostasDisponiveisList::onExibirReprovar');
        ...
        $action5->setDisplayCondition('PropostasDisponiveisList::onExibirAnexarArquivo');
        ...
        $panel = new TPanelGroup("Listagem de propostas {$manual}");
    ...
    public function onGerarItens($param = null) 
    {
        try 
        {
            ...
            $param = (array) $param;
            $propostaId = $param['id'] ?? ($param['key'] ?? null);

            if (empty($propostaId)) {
                new TMessage('error', 'Não foi possível identificar a proposta. Abra o registro e tente novamente.');
                return;
            }
            $object = new Propostas($propostaId, FALSE); 
            ...
            if ($itenspedido)
            {
                foreach ($itenspedido as $itensp)
                {
                    ...
                    if (!$itensx) {
                        $prod = new Produto($itensp->produto_id);
                        ...
                        $itenscotacao->familia_produto_id = $prod->familia_produto_id;
                        $itenscotacao->store();
            ...
            PropostasForm::recalcularTotaisDaProposta($object->id);
     ...
     public function onReprovar($param = null) 
    {
            ...
            if (isset($param['confirmReprovacao']) && $param['confirmReprovacao']) {
            try {
                ...
                $this->registrarHistoricoReprovacaoCotacao($object);
                TToast::show('success', "Proposta Reprovada(Rejeitada) com sucesso!!", 'topRight', 'far:check-circle');
                TApplication::loadPage('PropostasDisponiveisList', 'onReload');
                $this->onReload();
            ...
            // ConfirmaÃ§Ã£o antes de gerar a cotaÃ§Ã£o
            $action = new TAction(array($this, 'onReprovar'));
            $action->setParameters($param);
            $action->setParameter('confirmReprovacao', true);

            new TQuestion('Tem certeza que deseja Reprovar(Rejeitar) esta Proposta para Cotação?', $action);
        }
    }
    private function registrarHistoricoReprovacaoCotacao($propostas)
    {
        $histpropostas = new PropostasHistorico();
        $histpropostas->propostas_id = $propostas->id;
        $histpropostas->data_historico = date('Y-m-d H:i:s');
        $histpropostas->estado_pedido_frotas_id = EstadoPedidoFrotas::REPROVADO; 
    ...
    public static function onExibirGerar($object)
    {
        try 
        {
            TTransaction::open('minierp'); // ajuste o nome do banco se necessÃ¡rio

            // Carrega o pedido pelo ID vindo do objeto
            $pedido = new Propostas($object->id);

            // Define os estados permitidos para exibir o botÃ£o
            $estadosPermitidos = [
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->veiculos_id) AND ( (is_scalar($data->veiculos_id) AND $data->veiculos_id !== '') OR (is_array($data->veiculos_id) AND (!empty($data->veiculos_id)) )) )
        {
            ...
            $filters[] = new TFilter('veiculos_id', '=', $data->veiculos_id);// create filter 
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if ($pessoa) {
                // Assume que o usuÃ¡rio sÃ³ tem uma pessoa associada (se houver vÃ¡rias, pegarÃ¡ a Ãºltima)
                foreach ($pessoa as $pe) {
                    $codpessoa = $pe->id;
                    break;
                }
            
                // Se nÃ£o for grupo admin, aplicar filtro de pessoa
                if (!$grupo) {
                    $criteria->add(new TFilter('pessoa_id', '=', $codpessoa));
            
                    // CÃ¡lculo das taxas (com validaÃ§Ã£o de divisÃ£o)
    ...
    public function onShow($param = null)
    {
        ...
        {   
            ...
            if (isset($param['propostas_id'])) {
                ...
                if ($proposta) {
                    ...
                    foreach ($itenspropostas as $ip) {
                        ...
                        if (!$proposta_atual_foi_alterada) {
                            // 2. Copiar para outras propostas vÃ¡lidas, mas sÃ³ marcar revisÃ£o se inserido novo item
                            ...
                            foreach ($outras_propostas as $pr) {
                                ...
                                if (!$existe) {
                                    ...
                                    $propostas_alteradas[] = $pr->id; // marcar para revisÃ£o
                                }
                            }
                        }
                    ...
                    // 4. Marcar somente as propostas realmente alteradas como revisÃ£o
                    ...
                    // 5. Remover itens do pedido que nÃ£o estÃ£o mais em nenhuma proposta
                    ...
                    // 6. Marcar o PedidoFrotas como em revisÃ£o
                ...
                $propostasalteradas = Propostas::where('pedido_frotas_id', '=', $pedido_id)
                                                ->load();
                foreach ($propostasalteradas as $p) {
                    PropostasForm::recalcularTotaisDaProposta($p->id);
                }
    ...
    public static function onExibirAnexarArquivo($object)
    {
        try
        {
            return true;
