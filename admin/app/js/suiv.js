$(document).ready(function(){
    ...
    let placa_veiculo = $(".placa-veiculo").attr("rel");
    ...
    $(document).on("change", "#tipoProduto", function(){
        let valor = $(this).val();
        if(`${valor}` == "2"){
            $(".input-produto-ou-servico").attr("list", "lista-de-produtos");
        }else{
           $(".input-produto-ou-servico").removeAttr("list"); 
        }
    });

    consultar_placa(placa_veiculo);
    ...
    async function gerar_Sets(token){
        ...
        for(let i of result){
            ...
            if(gerar_nicknames_res['quantidade'] == result_quantidade){
                ...
                for(let x of arrays){
                    console.log(x);
                    $("#lista-de-produtos").append(`<option value="${x}">`);
                }
                $(".loading").hide();
            }else{
    ...
    async function gerar_pecas(token, id_peca, nome_peca){
        await $.ajax({
            ...
            success: function (data) {
                if(`${data}` != ""){
                    for(let i of data){
                        array_pecas.push(i.description);
                    }
                }else{
                    array_pecas.push(nome_peca);
                }
            },  
            error: function (data) {
                $(".loading").hide(); 
