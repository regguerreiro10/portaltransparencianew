...
class APICombustivel
{
    ...
    ): array
    {
        ...
        if ($response === false) {
            throw new Exception('Erro ao consultar contas');
        }
        ...
        foreach ($contas as $conta) {
            ...
            // 🔹 Chama API de fatura
            $faturas = self::getApp_RecuperarFaturas(
                $token,
                $grupo,
                $login,
                $senha,
            ...
            //recuperar lancamentos da faturas
             $xmlExternoF = simplexml_load_string($faturas);

            if ($xmlExternoF === false) {
                exit;
            }

            $xmlInternoStringF = html_entity_decode((string) $xmlExternoF, ENT_QUOTES, 'UTF-8');

            $xmlInternoF = simplexml_load_string($xmlInternoStringF);

            if ($xmlInternoF === false) {
                exit;
            }
            
            $cliente = (string) $xmlInternoF->cd_cliente;
            $rede    = (string) $xmlInternoF->cd_rede;

           //var_dump( $cliente, $rede);

            $faturas1 = $xmlInternoF->faturas->fatura;
            $resultado = [];
            foreach ($faturas1 as $fatura1) {
                $nu_seq_fatura = (string) $fatura1->nu_seq_fatura;

                $lancamentos = self::getApp_RecuperarFaturaComLancamentos(
                    $token,
                    $grupo,
                    $login,
                    $senha,
                ...
                $resultadolancamentosfaturas[] = [
                    'cliente'    => $cliente,
                    'rede'       => $rede,
                    'nu_seq_fatura' => $nu_seq_fatura,
                    'lancamentos' => $lancamentos
                ];
                // var_dump($resultadolancamentosfaturas);
            }
        }
        //fim do recuperar faturas
