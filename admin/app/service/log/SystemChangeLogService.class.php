...
{
    /**
     * Persist log row handling PK race conditions when IDPOLICY = max.
     */
    private static function storeLogWithRetry(SystemChangeLog $log, int $maxAttempts = 5): void
    {
        $attempt = 0;
        ...
        while (true)
        {
            ...
            {
                $attempt++;
                $isDuplicateKey = (stripos($e->getMessage(), 'SQLSTATE[23000]') !== false)
                ...
                // Small jitter to reduce max(id)+1 contention between concurrent requests.
                usleep(random_int(10000, 80000));
            }
        }
    }
